...
def train_epoch(epoch, wandb):
    ...
    # 思考标签
    start_think_ids = tokenizer('<think>').input_ids
    end_think_ids = tokenizer('</think>').input_ids
    start_answer_ids = tokenizer('<answer').input_ids
    end_answer_ids = tokenizer('</answer>').input_ids
    ...
    for step, (X, Y, loss_mask) in enumerate(train_loader):
        ...
        with ctx:
            ...
            # 扩大特殊token的损失
            special_ids = torch.isin(Y.view(-1), torch.tensor(start_think_ids + end_think_ids + start_answer_ids + end_answer_ids).to(args.device))
